#!/usr/bin/env python3
"""Generate per-case config.yaml files for the OAT-DRAINED sweep.

Same OAT study as OAT-SEALED but the right boundary is drained (P=0) and
mechanically free — allowing lateral drainage and radial expansion.
...
def _make_config(label, perm, alpha, phi):
    return {
        "general": {
            "description": f"OAT-DRAINED {label}",
            "tags":        "oat_sensitivity drained",
            "run_dir":     "",
            "run_id":      label,
        },
...
def main():
    ...
    cases = _all_cases()
    print(f"OAT-DRAINED — {len(cases)} cases → {RUNS_DIR.relative_to(DEMO_DIR)}/")

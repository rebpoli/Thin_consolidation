#!/usr/bin/env python3
"""01-Traditional-Consolidation — FEM vs Analytical comparison plot.

Loads fem_timeseries.nc and analytical_timeseries.nc from outputs/ and
produces a side-by-side comparison of pressure dissipation and axial
displacement convergence against the 1D Terzaghi analytical solution.
...
def main():
    ...
    if ana is not None and "pressure_mean" in ana:
        ...
        ax_p.plot(t[mask] / 60, p_ana, "k--", lw=1.2, label="Analytical")
    ...
    if "pressure_l2_error" in fem:
        ...
        ax_err.plot(t[mask] / 60, err, color="firebrick", lw=1.5, label="Pressure L2 error vs analytical")
        ax_err.set_xscale("log"); ax_err.set_yscale("log")
        ax_err.set_xlabel("Time [min]", fontsize=8)
        ax_err.set_ylabel("L2 error [Pa]", fontsize=8)
        ax_err.set_title("FEM vs Analytical L2 error", fontsize=9)
        ax_err.legend(fontsize=7); ax_err.grid(True, which="both", alpha=0.25)
        ax_err.tick_params(labelsize=7)
    else:
        ax_err.set_visible(False)

    fig.suptitle("01-Traditional-Consolidation — FEM vs Terzaghi Analytical\n"
                 "Jacketed test (sealed side U_r=0), drained top, cyclic axial load",
                 fontsize=10)

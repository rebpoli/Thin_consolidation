...
def _mc_envelope(phi_rad, coh_MPa, p_max_MPa, npoints=300):
...
def _plot_run(run_label):
    ...
    r      = ds["r"].values               # [m]
    z      = ds["z"].values               # [m]
    p      = -ds["p_eff"].values / 1e6    # -p'     [MPa], positive = compression
    ...
    # Flatten for scatter
    n_t, n_pt = p.shape
    time_mat  = np.repeat(time[:, np.newaxis], n_pt, axis=1)
    ...
    ax.legend(loc="upper left", framealpha=0.7, handlelength=1.5)

    cbar = plt.colorbar(sc, cax=ax_cbar, drawedges=False)
    cbar.solids.set_alpha(1.0)
    cbar.set_label("Time (min)", rotation=270, labelpad=10)

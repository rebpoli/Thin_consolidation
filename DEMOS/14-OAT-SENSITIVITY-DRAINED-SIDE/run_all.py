#!/usr/bin/env python3
"""Demo 14 — OAT sensitivity study with drained lateral boundary.

Same as Demo 13 but the right boundary (r=Re) has P=0 (drained) and is
mechanically free (no U_r constraint), allowing lateral drainage and expansion.
...
Three independent sweeps:
  ...
  alpha : {0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 1.0} fixed phi=0.10, perm=1e-20
...
def _make_config(label, perm, alpha, phi):
    ...
    return {
        "general": {
            ...
            "tags":        "oat_sensitivity demo14 drained_side",
            "run_dir":     "",
            "run_id":      label,
        },
        ...
        "boundary_conditions": {
            ...
            "top":    {
                ...
                "U_r":           0.0,            # no lateral displacement (rigid plate)
...
def main():
    ...
    print(f"\n{BOLD}Demo 14 — OAT Sensitivity (drained side) — {len(ALL_CASES)} parallel runs{RESET}")

#!/usr/bin/env python
"""M sweep: 5 parallel consolidation runs varying Biot modulus M in log-scale.

M:      logspace from 1e8 to 1.5e10 (5 steps)
alpha:  0.75 (fixed)
Loads:  L0=0 MPa (relaxed, t<50s)  L1=1 MPa  L2=5 MPa  period=100s  5 cycles
Logs:   log/M_<value>.log
Output: runs/M_<value>/outputs/
"""
import sys
import time
import subprocess
...
def _make_cases():
    ...
    cases = []
    for M in M_values:
        S          = 1/M + _alpha**2 * (1 - 2*_nu) / (2 * _mu * (1 - _nu))
        c_v        = _perm / (_visc * S)
        end_time_tv = _T_TARGET * c_v / (4 * _H**2)
        cases.append({"M": float(M), "end_time_tv": float(end_time_tv)})
    return cases

CASES = _make_cases()

def _fmt_M(M: float) -> str:
    """Format M as a compact string suitable for directory names, e.g. '3.5e8'."""
    s = f"{M:.2e}"
...
def make_config(M: float, end_time_tv: float) -> dict:
    return {
        "general": {
            "description": f"M sweep M={_fmt_M(M)} alpha={_alpha}",
            "tags": "M_sweep periodic",
            "run_dir": "",
            "run_id":  f"M_{_fmt_M(M)}",
        },
...
def main():
    ...
    print(f"\n{BOLD}M sweep — launching {len(CASES)} parallel runs{RESET}")
    print(f"  main  : {SRC_RUN}")
    print(f"  logs  : {LOG_DIR.relative_to(DEMO_DIR)}/")
    print(f"  alpha : {_alpha}  (fixed)")
    print(f"  M     : logspace {_fmt_M(CASES[0]['M'])} → {_fmt_M(CASES[-1]['M'])}")
    print()

    jobs = []
    for case in CASES:
        M        = case["M"]
        run_dir  = RUNS_DIR / f"M_{_fmt_M(M)}"
        log_path = LOG_DIR  / f"M_{_fmt_M(M)}.log"

        run_dir.mkdir(parents=True, exist_ok=True)
        (run_dir / "outputs").mkdir(exist_ok=True)
        _write_config(run_dir / "config.yaml", make_config(M, case["end_time_tv"]))

        job = Job(M=M, log=log_path)
        ...
        job.start = time.time()
        jobs.append(job)
        print(f"  {CYAN}▶{RESET} {job.name:<12}  → PID {job.proc.pid}"
